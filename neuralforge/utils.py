...
#method to show the gradient flow
def plot_gradient_flow(grad_magnitudes, iterations):
    """
    Plots the gradient magnitudes of weights during training.

    @param grad_magnitudes (dict): A dictionary where keys are weight names
                                   (e.g., 'w1', 'w2') and values are lists of gradient magnitudes.
    @param iterations (list): List of iteration numbers.
    """
    for weight_name, magnitudes in grad_magnitudes.items():
        plt.plot(iterations, magnitudes, label=f"{weight_name} Gradient Magnitude")
    
    plt.xlabel("Iteration")
    plt.ylabel("Gradient Magnitude")
    plt.title("Gradient Flow Through Weights")
    plt.legend()
    plt.show()